        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-[72px]">
              ...
              {navigation.map((item) => (
                <a
                  ...
                  className={`relative px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 ${
                    activeId === item.id ? "text-white" : "text-text-secondary hover:text-text-primary"
                  }`}
                >
                  {activeId === item.id && (
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <a href="#contact" className="magnetic-btn ml-1 px-5 py-2.5 text-sm font-semibold bg-gradient-to-r from-electric-500 to-electric-400 text-white rounded-xl shadow-lg shadow-electric-500/20 hover:shadow-electric-500/35 hover:scale-105 transition-all duration-300">
        ...
        <div className="fixed inset-0 z-40 bg-navy-950/98 backdrop-blur-2xl md:hidden" role="dialog" aria-modal="true" aria-label="Mobile navigation">
          <div className="flex flex-col items-center justify-center h-full gap-5 px-6">
            ...
            {navigation.map((item, i) => (
              <a key={item.id} href={item.href} onClick={() => setMobileOpen(false)} className={`text-2xl font-semibold transition-all duration-500 animate-slide-up ${activeId === item.id ? "text-electric-400" : "text-text-secondary"}`} style={{ animationDelay: `${i * 80 + 80}ms` }}>
                {item.label}
              </a>
            ))}
