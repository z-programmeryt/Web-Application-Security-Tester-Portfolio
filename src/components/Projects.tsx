...
function FeaturedProjectCard({ project, onSelect, inView }: { project: Project; onSelect: (p: Project) => void; inView: boolean }) {
  ...
  const combinedMove = (e: React.MouseEvent<HTMLDivElement>) => { tiltMove(e); spotMove(e); };

  return (
    <article
      ref={(el: HTMLDivElement | null) => { tiltRef.current = el; spotRef.current = el; }}
      onMouseMove={combinedMove}
      onMouseLeave={tiltLeave}
      className={`group relative glass-card spotlight overflow-hidden ${inView ? "animate-slide-up" : "opacity-0"}`}
        ...
        <div className="flex flex-col md:flex-row md:items-start gap-6">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="px-3 py-1 text-xs font-medium bg-electric-500/10 text-electric-300 border border-electric-500/15 rounded-full">
                {project.categoryLabel}
              </span>
            </div>

            <h3 className="text-2xl sm:text-3xl font-bold text-text-primary mb-3 group-hover:text-electric-300 transition-colors">
              {project.title}
            </h3>
            ...
            <div className="flex flex-wrap gap-1.5 mb-6">
              {project.technologies.slice(0, 6).map((t) => (
                <span key={t} className="px-2.5 py-1 text-[11px] font-mono bg-navy-800/50 border border-border-subtle rounded-md text-text-muted">
                  {t}
                </span>
              ))}
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="magnetic-btn inline-flex items-center gap-2 px-5 py-2.5 bg-electric-500 hover:bg-electric-400 text-white text-sm font-semibold rounded-xl shadow-lg shadow-electric-500/20 transition-all"
              >
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 border border-border-medium hover:border-electric-500/40 text-sm font-medium rounded-xl transition-all hover:bg-electric-500/5"
              >
...
function ProjectCard({ project, onSelect, index }: { project: Project; onSelect: (p: Project) => void; index: number }) {
  const { ref: spotRef, handleMouseMove: spotMove } = useSpotlight();
  ...
  return (
    <article
      ...
      className="group glass-card spotlight overflow-hidden hover:scale-[1.02] transition-all duration-400 animate-slide-up flex flex-col"
      style={{ animationDelay: `${index * 100}ms` }}
    >
      <div className="p-5 sm:p-6 flex-1 flex flex-col">
        <div className="flex items-center gap-2 mb-3">
          <span className="px-2.5 py-0.5 text-[10px] font-medium bg-electric-500/10 text-electric-300 border border-electric-500/15 rounded-full">
            {project.categoryLabel}
          </span>
        ...
        <h3 className="text-lg font-bold text-text-primary group-hover:text-electric-300 transition-colors mb-2">
          {project.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {project.languages.map((lang) => (
            <span key={lang} className="px-2 py-0.5 text-[10px] font-mono bg-navy-800/50 border border-border-subtle rounded text-text-muted">
              {lang}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between pt-4">
          <button
            onClick={() => onSelect(project)}
            className="flex items-center gap-1 text-sm text-electric-400 hover:text-electric-300 transition-colors font-medium"
          >
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="section-padding relative" ref={setRef}>
      <div className="max-w-7xl mx-auto">
        <div className={`max-w-2xl mb-10 ${inView ? "animate-slide-up" : "opacity-0"}`}>
          <span className="text-electric-400 font-mono text-xs tracking-widest uppercase mb-3 block">Projects</span>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-5">
            Featured <span className="text-gradient">Projects</span>
          </h2>
          <p className="text-text-secondary text-lg">Security tooling and research — reflecting practical cybersecurity skills.</p>
          <div className="w-16 h-0.5 bg-gradient-to-r from-electric-500 to-cyan-accent rounded-full mt-5" />
        </div>

        {/* Filters */}
        <div className={`flex flex-wrap gap-2 mb-8 ${inView ? "animate-slide-up" : "opacity-0"}`} style={{ animationDelay: "80ms" }}>
        ...
        {/* Featured project */}
        {featured && <div className="mb-6"><FeaturedProjectCard project={featured} onSelect={setSelectedProject} inView={inView} /></div>}

        {/* Grid */}
        {rest.length > 0 && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {rest.map((project, i) => (
              <ProjectCard key={project.slug} project={project} onSelect={setSelectedProject} index={i} />
            ))}
          </div>
        )}
        ...
        <div className={`text-center mt-10 ${inView ? "animate-fade-in" : "opacity-0"}`} style={{ animationDelay: "400ms" }}>
          <a href="https://github.com/redoyrowshon-dotcom" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-text-secondary hover:text-electric-400 transition-colors">
